...
} from "./lib/aiSixPictureStoriesPublication20260910Artifact";
import {
  ...
  SIX_PICTURE_STORIES_POLICY_VERSION as policyVersion,
  SIX_PICTURE_STORIES_PREIMAGE as preimage,
  SIX_PICTURE_STORIES_RELEASE_DAYS as releaseDays,
  SIX_PICTURE_STORIES_RELEASE_ROOT as root,
  SIX_PICTURE_STORIES_SLUGS as slugs,
  sixPictureStoriesReleaseId,
  sixPictureStoriesContentRunId,
  sixPictureStoriesSourceRunId,
} from "./lib/aiSixPictureStoriesPublication20260910Data";
...
const nextAuditDate = new Date(
  ...
  .toISOString()
  .slice(0, 10);
export const SIX_PICTURE_STORIES_VISIBILITY_CUTOFF =
...
const action = (phase: string) => `library.ai_six_picture_stories.${phase}`;
...
function assertIdentity(args: typeof identity) {
  if (!equal(args, identity))
    throw new Error("Six-picture-story release identity mismatch");
}
...
function assertFresh(now: number) {
  if (
    ...
    now >= SIX_PICTURE_STORIES_VISIBILITY_CUTOFF
  ) {
...
async function assertCompiledEvidence() {
  if (
    ...
    throw new Error("Compiled six-picture-story identity or evidence drift");
  }
  for (const [index, target] of artifact.targets.entries()) {
    ...
    if (
      ...
      expected.desiredRevision !== [6, 6, 7, 6, 6, 6][index] ||
      !equal(expected.sourceIds, ["aap-power-of-play-2018", "cdc-milestones-2026"]) ||
      target.verdict !== "pass" ||
      target.contentSnapshotHash !== expected.desiredContentSnapshotHash ||
      target.evidenceLinkSnapshotHash !== expected.linkSnapshotHash ||
      target.mediaCount !== 2 ||
      expected.mediaCount !== 2 ||
      ...
      throw new Error("Compiled six-picture-story target evidence drift");
    }
  }
}
...
function targetLane(index: number) {
  const expected = preimage.targets[index],
    ...
    slug = slugs[index];
  const releaseId = sixPictureStoriesReleaseId(slug),
    contentRunId = sixPictureStoriesContentRunId(slug);
  const sourceRunId = (sourceId: string) =>
    sixPictureStoriesSourceRunId(slug, sourceId);
  ...
  async function auditPayloads(
    ...
    const summary = `${artifact.summary} Target: activity:${slug}.`;
  ...
  function releasePayload(
    ...
    return {
      releaseId,
      targetKey: aiPublicationTargetKey("activity", slug),
    ...
    const [
      ...
      ctx.db
        ...
        .withIndex("by_target_key", (q) =>
          q.eq("targetKey", aiPublicationTargetKey("activity", slug)),
        )
        .take(2),
      ctx.db
        .query("aiAuditRuns")
  ...
  async function inspect(
    ...
    if (
      ...
      isRegisteredReleaseContentTarget("activity", slug) ||
    ...
    if (enabled) {
      ...
      expiryScheduleExact = Boolean(
        schedule &&
        schedule.name === "aiSixPictureStoriesPublication20260910.js:expire" &&
        schedule.scheduledTime === SIX_PICTURE_STORIES_VISIBILITY_CUTOFF &&
...
async function inspectBatch(ctx: Ctx, now: number) {
  ...
  if (phase === "drift" && !blockers.length)
    blockers.push("Six-item lifecycle is not atomic");
  return {
    targets,
    report: {
      ...identity,
...
/** Correct all six exact authored copies and record real AI reports atomically; all six stay dark. */
export const stage = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (before.report.phase !== "ready")
      throw new Error(
        "Six-picture-story stage blocked: " + before.report.blockers.join("; "),
      );
    const staged = {
...
/** Publish exactly six AI previews together; no human decision or source row changes. */
export const activate = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (before.report.phase !== "staged")
      throw new Error(
        "Six-picture-story activation blocked: " + before.report.blockers.join("; "),
      );
    const expiryScheduledFunctionId = await ctx.scheduler.runAt(
      SIX_PICTURE_STORIES_VISIBILITY_CUTOFF,
      internal.aiSixPictureStoriesPublication20260910.expire,
      identity,
    );
    const enabled = {
      operator: args.operator,
    ...
    for (const [index, entry] of before.targets.entries()) {
      ...
      await ctx.db.patch(entry.content._id, {
        aiPublicationReleaseId: sixPictureStoriesReleaseId(entry.target.slug),
        aiPublishedAt: now,
      });
    }
    await logAudit(
    ...
    if (
      ...
      throw new Error(
        "Atomic 24-item aggregate readback failed; transaction rolls back",
      );
    return after.report;
  },
});
async function terminate(
  ...
  for (const expected of preimage.targets) {
    const releaseId = sixPictureStoriesReleaseId(expected.slug);
    const rows = await ctx.db
      .query("aiPublicationReleases")
      .withIndex("by_release_id", (q) => q.eq("releaseId", releaseId))
      .take(2);
    if (rows.length > 1) throw new Error("Duplicate exact six-picture-story release");
    const release = rows[0];
    if (
      release &&
      (release.contentId !== expected.contentId ||
        release.contentSlug !== expected.slug ||
        release.contentType !== "activity" ||
        release.auditArtifactHash !== artifactHash ||
        release.policyVersion !== policyVersion ||
        release.targetKey !== aiPublicationTargetKey("activity", expected.slug))
    )
    ...
    if (release?.status === "active") {
      await ctx.db.patch(release._id, {
        ...
        revokedAt: now,
        revokeReason:
          "Exact six-picture-story AI release " +
          reason +
...
export const expire = internalMutation({
  ...
  handler: async (ctx, args) => {
    assertIdentity(args);
    if (Date.now() < SIX_PICTURE_STORIES_VISIBILITY_CUTOFF)
