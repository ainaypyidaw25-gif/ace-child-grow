...
import {
  ...
  ENGLISH_REFREEZE_CORRECTION_EXPIRES_AT,
...
function auditBeforeJson(): string {
  return JSON.stringify({
    fixtureSha256: ENGLISH_REFREEZE_CORRECTION_FIXTURE_SHA256,
    predecessorBatchId: ENGLISH_REFREEZE_PREDECESSOR_BATCH_ID,
    stoppedDecisionSetDigest: ENGLISH_REFREEZE_DECISION_SET_DIGEST,
    targets: ENGLISH_REFREEZE_CORRECTION_TARGETS.map((target) => ({
      kind: target.kind,
      slug: target.slug,
...
function auditAfterJson(updatedAt: number, postimages: readonly AuditedPostimage[]): string {
  return JSON.stringify({
    ...
    targets: ENGLISH_REFREEZE_CORRECTION_TARGETS.map((target) => {
      ...
      if (!postimage) throw new Error(`Missing English refreeze postimage: ${target.slug}`);
      return {
        kind: target.kind,
        slug: target.slug,
        contentId: target.content._id,
    ...
    confirmedMyanmarCopyPaths: [
      'guide:gd_5_6m_nutrition.titleMm',
      'guide:gd_5_6m_nutrition.summaryMm',
      'guide:gd_5_6m_nutrition.data.title.mm',
      'guide:gd_5_6m_nutrition.data.why.mm',
    ],
...
async function releaseAuditState(ctx: DatabaseContext) {
  const rows = await ctx.db.query('auditLogs')
    .withIndex('by_action', (q) => q.eq('action', ENGLISH_REFREEZE_CORRECTION_ACTION))
    .take(2);
  if (rows.length !== 1) {
    return {
      rows: rows.length,
  ...
  const exact = updatedAt !== null
    ...
    && postimages.length === ENGLISH_REFREEZE_CORRECTION_TARGETS.length
    ...
    && row.summary === ENGLISH_REFREEZE_CORRECTION_RELEASE_ID
    && row.result === 'ok'
...
async function registryAndDecisionState(ctx: DatabaseContext) {
  ...
  const fixtureRegistry = ENGLISH_REFREEZE_CORRECTION_PREIMAGES.registry;
  ...
  const predecessor = batches.find(
    (row) => row.batchId === ENGLISH_REFREEZE_PREDECESSOR_BATCH_ID,
  );
  const decisions = [];
  for (const target of ENGLISH_REFREEZE_CORRECTION_TARGETS) {
    const assignment = assignments.find((row) => (
      row.batchId === ENGLISH_REFREEZE_PREDECESSOR_BATCH_ID
  ...
  const digest = predecessor ? await sha256Canonical({
    batchId: ENGLISH_REFREEZE_PREDECESSOR_BATCH_ID,
    freezeDigest: predecessor.freezeDigest,
    decisions,
  }) : null;
  return {
    registryExact,
    decisionSetExact: digest === ENGLISH_REFREEZE_DECISION_SET_DIGEST,
  };
}

function contentIdentityMatches(
  row: Doc<'libraryContent'>,
  target: EnglishRefreezeCorrectionTarget,
): boolean {
...
function desiredContentMatches(
  ...
  return canonicalJson(authoredSnapshot(row)) === canonicalJson(authoredSnapshot(target.desiredContent))
    ...
    && sameStrings(row.requiredReviewDimensions ?? [], ENGLISH_REFREEZE_REQUIRED_REVIEWS)
...
async function inspectTarget(
  ...
  return {
    ...
    outstandingRequiredReviews: ENGLISH_REFREEZE_REQUIRED_REVIEWS
      .filter((dimension) => !approvedDimensions.has(dimension)),
  };
}

export async function englishRefreezeCorrectionPreflightState(
  ctx: DatabaseContext,
  checkedAt: number,
): Promise<EnglishRefreezeCorrectionPreflight> {
  ...
  const [fixtureHash, audit, registry] = await Promise.all([
    sha256Canonical(ENGLISH_REFREEZE_CORRECTION_PREIMAGES),
    releaseAuditState(ctx),
    registryAndDecisionState(ctx),
  ]);
  const fixtureExact = fixtureHash === ENGLISH_REFREEZE_CORRECTION_FIXTURE_SHA256;
  const targets = [];
  for (const target of ENGLISH_REFREEZE_CORRECTION_TARGETS) {
  ...
  if (audit.rows === 0 && checkedAt >= ENGLISH_REFREEZE_CORRECTION_EXPIRES_AT) {
...
export const preflightAt = internalQuery({
  ...
  handler: async (ctx, args) => englishRefreezeCorrectionPreflightState(ctx, args.checkedAt),
});
...
function desiredPatch(
  ...
  return {
    ...
    requiredReviewDimensions: [...ENGLISH_REFREEZE_REQUIRED_REVIEWS],
    reviewRevision: target.desiredReviewRevision,
    clinicalStatus: 'clinical_review',
...
export const apply = internalMutation({
  ...
  handler: async (ctx) => {
    const now = Date.now();
    const before = await englishRefreezeCorrectionPreflightState(ctx, now);
    ...
    if (before.phase !== 'ready') {
      throw new Error(`English refreeze correction blocked: ${before.blockers.join('; ')}`);
    }
    const rechecked = await englishRefreezeCorrectionPreflightState(ctx, now);
    ...
    for (const target of ENGLISH_REFREEZE_CORRECTION_TARGETS) {
      ...
      if (!row) throw new Error(`English refreeze postimage disappeared: ${target.slug}`);
    ...
    const after = await englishRefreezeCorrectionPreflightState(ctx, now);
    if (after.phase !== 'applied') {
      throw new Error('English correction postflight failed; transaction rolled back');
    }
    return {
      releaseId: ENGLISH_REFREEZE_CORRECTION_RELEASE_ID,
      applied: true,
      alreadyApplied: false,
      contentUpdated: ENGLISH_REFREEZE_CORRECTION_TARGETS.length,
      semanticContentUpdated: 1,
      revisionOnlyContentUpdated: ENGLISH_REFREEZE_CORRECTION_TARGETS.length - 1,
      linksUpdated: 0 as const,
      publicationDecisionMade: false as const,
      updatedAt: now,
    };
