...
import {
  TWO_LESSONS_ARTIFACT as artifact,
  TWO_LESSONS_ARTIFACT_HASH as artifactHash,
} from "./lib/aiTwoLessonsPublication20260910Artifact";
import {
  TWO_LESSONS_DESIRED_FIELDS as desiredFields,
  TWO_LESSONS_PRESERVATION as preservation,
  TWO_LESSONS_OLD_SCHEDULES as oldSchedules,
  TWO_LESSONS_POLICY_VERSION as policyVersion,
  TWO_LESSONS_PREIMAGE as preimage,
  TWO_LESSONS_RELEASE_DAYS as releaseDays,
  TWO_LESSONS_RELEASE_ROOT as root,
  TWO_LESSONS_SLUGS as slugs,
  twoLessonsReleaseId,
  twoLessonsContentRunId,
  twoLessonsSourceRunId,
} from "./lib/aiTwoLessonsPublication20260910Data";
...
const nextAuditDate = new Date(
  ...
  .toISOString()
  .slice(0, 10);
export const TWO_LESSONS_VISIBILITY_CUTOFF =
...
const action = (phase: string) => `library.ai_two_lessons.${phase}`;
...
function assertIdentity(args: typeof identity) {
  if (!equal(args, identity))
    throw new Error("Two-lesson release identity mismatch");
}
...
function assertFresh(now: number) {
  if (
    ...
    now >= TWO_LESSONS_VISIBILITY_CUTOFF
  ) {
...
async function assertCompiledEvidence() {
  if (
    ...
    !equal(slugs, ["lsn_what_is_development", "lsn_big_feelings"])
  ) {
    throw new Error("Compiled two-lesson identity or evidence drift");
  }
  for (const [index, target] of artifact.targets.entries()) {
    ...
    if (
      ...
      expected.desiredRevision !== [3, 5][index] ||
      !equal(
        expected.sourceIds,
        index === 0
      ...
      throw new Error("Compiled two-lesson target evidence drift");
    }
  }
}
...
function targetLane(index: number) {
  const expected = preimage.targets[index],
    ...
    slug = slugs[index];
  const releaseId = twoLessonsReleaseId(slug),
    contentRunId = twoLessonsContentRunId(slug);
  const sourceRunId = (sourceId: string) =>
    twoLessonsSourceRunId(slug, sourceId);
  ...
  async function inspect(ctx: Ctx, now: number) {
              ...
              !preimage.targets.some((entry) =>
                entry.sourceIds.some(
                  (sourceId) =>
                    ...
                    row.runId === twoLessonsSourceRunId(entry.slug, sourceId),
                ),
              ),
          );
        } else {
    ...
    if (!preservationChecks.every(Boolean) || !scheduleChecks.every(Boolean)) {
      blockers.push(
        "Exact previous-twelve dependency/history/audit/schedule preservation drift",
      );
    }
    if (
      !Number.isFinite(now) ||
    ...
    if (enabled) {
      ...
      expiryScheduleExact = Boolean(
        schedule &&
        schedule.name === "aiTwoLessonsPublication20260910.js:expire" &&
        schedule.scheduledTime === TWO_LESSONS_VISIBILITY_CUTOFF &&
...
async function inspectBatch(ctx: Ctx, now: number) {
  ...
  if (phase === "drift" && !blockers.length)
    blockers.push("Two-item lifecycle is not atomic");
  return {
    targets,
    report: {
      ...identity,
...
/** Correct both exact authored copies and record real AI reports atomically; both stay dark. */
export const stage = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (before.report.phase !== "ready")
      throw new Error(
        "Two-lesson stage blocked: " + before.report.blockers.join("; "),
      );
    const staged = {
    ...
    if (
      ...
      after.report.activeReleaseCount !== 12
    )
      throw new Error("Atomic stage postflight failed; transaction rolls back");
    return after.report;
  },
});
/** Publish exactly two AI previews together; no human decision or source row changes. */
export const activate = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (before.report.phase !== "staged")
      throw new Error(
        "Two-lesson activation blocked: " + before.report.blockers.join("; "),
      );
    const expiryScheduledFunctionId = await ctx.scheduler.runAt(
      TWO_LESSONS_VISIBILITY_CUTOFF,
      internal.aiTwoLessonsPublication20260910.expire,
      identity,
    );
    const enabled = {
      operator: args.operator,
    ...
    for (const [index, entry] of before.targets.entries()) {
      ...
      await ctx.db.patch(entry.content._id, {
        aiPublicationReleaseId: twoLessonsReleaseId(entry.target.slug),
        aiPublishedAt: now,
      });
    }
    await logAudit(
    ...
    if (
      ...
      after.report.activeReleaseCount !== 14
    )
      throw new Error(
        "Atomic fourteen-item readback failed; transaction rolls back",
      );
    return after.report;
  },
});
async function terminate(
  ...
  for (const expected of preimage.targets) {
    const releaseId = twoLessonsReleaseId(expected.slug);
    const rows = await ctx.db
      .query("aiPublicationReleases")
      .withIndex("by_release_id", (q) => q.eq("releaseId", releaseId))
      .take(2);
    if (rows.length > 1) throw new Error("Duplicate exact two-lesson release");
...
export const expire = internalMutation({
  ...
  handler: async (ctx, args) => {
    assertIdentity(args);
    if (Date.now() < TWO_LESSONS_VISIBILITY_CUTOFF)
