...
import {
  ...
  type Birth2mGrossMotorSourcePreimage,
} from './lib/birth2mGrossMotorCasData';
...
const releaseAction = 'release.birth_2m_gross_motor_content_evidence_correction';
...
const preflightResultValidator = v.object({
  releaseId: v.literal(BIRTH2M_GROSS_MOTOR_CAS_RELEASE_ID),
...
async function sourceMatches(
  ...
  expected: Birth2mGrossMotorSourcePreimage,
): Promise<boolean> {
...
function contentIdentityMatches(row: Doc<'libraryContent'>): boolean {
  return String(row._id) === BIRTH2M_GROSS_MOTOR_TARGET.contentId
    && row._creationTime === BIRTH2M_GROSS_MOTOR_TARGET.contentCreationTime
    && row.type === BIRTH2M_GROSS_MOTOR_TARGET.kind
    && row.slug === BIRTH2M_GROSS_MOTOR_TARGET.slug;
}

function linkIdentityMatches(row: Doc<'evidenceLinks'>): boolean {
  return String(row._id) === BIRTH2M_GROSS_MOTOR_TARGET.linkId
    && row._creationTime === BIRTH2M_GROSS_MOTOR_TARGET.linkCreationTime
    && row.createdAt === BIRTH2M_GROSS_MOTOR_TARGET.linkCreatedAt
    && row.kind === BIRTH2M_GROSS_MOTOR_TARGET.kind
    && row.slug === BIRTH2M_GROSS_MOTOR_TARGET.slug;
}
...
function auditBeforeJson(): string {
  return JSON.stringify({
    content: {
      rowId: BIRTH2M_GROSS_MOTOR_TARGET.contentId,
      canonicalSha256: BIRTH2M_GROSS_MOTOR_TARGET.contentInitialCanonicalSha256,
      reviewRevision: BIRTH2M_GROSS_MOTOR_TARGET.contentInitialReviewRevision,
      updatedAt: BIRTH2M_GROSS_MOTOR_TARGET.contentInitialUpdatedAt,
    },
    link: {
      rowId: BIRTH2M_GROSS_MOTOR_TARGET.linkId,
      canonicalSha256: BIRTH2M_GROSS_MOTOR_TARGET.linkInitialCanonicalSha256,
      sourceIds: BIRTH2M_GROSS_MOTOR_TARGET.initialSourceIds,
      updatedAt: BIRTH2M_GROSS_MOTOR_TARGET.linkInitialUpdatedAt,
    },
    sources: BIRTH2M_GROSS_MOTOR_SOURCE_PREIMAGES.map((row) => ({
      sourceId: row.sourceId,
      rowId: row.rowId,
      canonicalSha256: row.exactCanonicalSha256,
    })),
    reviewHistory: BIRTH2M_GROSS_MOTOR_REVIEW_PREIMAGES.map((row) => ({
      rowId: row.rowId,
      canonicalSha256: row.exactCanonicalSha256,
    })),
    media: [],
...
function auditAfterJson(
  ...
  return JSON.stringify({
    content: {
      ...
      reviewRevision: BIRTH2M_GROSS_MOTOR_TARGET.contentDesiredReviewRevision,
      updatedAt,
    },
    link: {
      rowId: BIRTH2M_GROSS_MOTOR_TARGET.linkId,
    ...
    sourcesPreserved: BIRTH2M_GROSS_MOTOR_SOURCE_PREIMAGES.map((row) => ({
      sourceId: row.sourceId,
      rowId: row.rowId,
      canonicalSha256: row.exactCanonicalSha256,
    })),
    mediaPreserved: [],
    reviewsPreserved: BIRTH2M_GROSS_MOTOR_REVIEW_PREIMAGES.map((row) => ({
      rowId: row.rowId,
      canonicalSha256: row.exactCanonicalSha256,
    })),
...
async function releaseAuditState(ctx: DatabaseContext): Promise<AuditState> {
  ...
  const exact = updatedAt !== null
    ...
    && row.summary === BIRTH2M_GROSS_MOTOR_CAS_RELEASE_ID
    && row.result === 'ok'
...
async function preflightState(ctx: DatabaseContext, now: number) {
  ...
  const [
    ...
    ctx.db.query('libraryContent').withIndex('by_slug', (q) => q
      .eq('slug', BIRTH2M_GROSS_MOTOR_TARGET.slug)).take(2),
    ctx.db.query('evidenceLinks').withIndex('by_kind_slug', (q) => q
      .eq('kind', BIRTH2M_GROSS_MOTOR_TARGET.kind)
      .eq('slug', BIRTH2M_GROSS_MOTOR_TARGET.slug)).take(2),
    ctx.db.query('libraryMedia').withIndex('by_content', (q) => q
      .eq('contentSlug', BIRTH2M_GROSS_MOTOR_TARGET.slug))
      .take(BIRTH2M_GROSS_MOTOR_MEDIA_PREIMAGES.length + 1),
    ctx.db.query('contentReviews').withIndex('by_content', (q) => q
      .eq('contentSlug', BIRTH2M_GROSS_MOTOR_TARGET.slug))
      .take(BIRTH2M_GROSS_MOTOR_REVIEW_PREIMAGES.length + 1),
    ctx.db.query('aiPublicationReleases').withIndex('by_target_key', (q) => q
      .eq('targetKey', `${BIRTH2M_GROSS_MOTOR_TARGET.kind}:${BIRTH2M_GROSS_MOTOR_TARGET.slug}`))
      .take(1),
  ...
  const sourceResults = await Promise.all(BIRTH2M_GROSS_MOTOR_SOURCE_PREIMAGES.map(
    async (expected) => {
      const rows = await ctx.db
  ...
  const sourcesById = new Map(sourceResults.flatMap((result) => result.row
    ...
    : []));
  const desiredSources = BIRTH2M_GROSS_MOTOR_TARGET.desiredSourceIds.flatMap((sourceId) => {
  ...
  const citationEligible = evaluatePublicationEvidence(
    BIRTH2M_GROSS_MOTOR_TARGET.desiredSourceIds,
    desiredSources,
    todayIso,
  ).allowed;
  ...
  const [contentPreimageExact, linkPreimageExact] = await Promise.all([
    content ? rowMatches(content, BIRTH2M_GROSS_MOTOR_CONTENT_PREIMAGE) : false,
    link ? rowMatches(link, BIRTH2M_GROSS_MOTOR_LINK_PREIMAGE) : false,
  ]);
  const contentDesiredPayloadMatches = Boolean(content
    && content.titleMm === BIRTH2M_GROSS_MOTOR_DESIRED_TITLE_MM
    && content.titleEn === BIRTH2M_GROSS_MOTOR_DESIRED_TITLE_EN
    && canonicalJson(content.data) === canonicalJson(BIRTH2M_GROSS_MOTOR_DESIRED_DATA)
    && content.searchText === BIRTH2M_GROSS_MOTOR_DESIRED_SEARCH_TEXT);
  const contentInitialMatches = Boolean(content
    ...
    && contentHash === BIRTH2M_GROSS_MOTOR_TARGET.contentInitialCanonicalSha256);
  const contentDesiredMatches = Boolean(content
    ...
    && content.reviewRevision === BIRTH2M_GROSS_MOTOR_TARGET.contentDesiredReviewRevision
  ...
  const linkInitialMatches = Boolean(link
    ...
    && linkPreimageExact
    && link.updatedAt === BIRTH2M_GROSS_MOTOR_TARGET.linkInitialUpdatedAt
    && linkHash === BIRTH2M_GROSS_MOTOR_TARGET.linkInitialCanonicalSha256
    && sameStrings(link.sourceIds, BIRTH2M_GROSS_MOTOR_TARGET.initialSourceIds));
  const linkDesiredMatches = Boolean(link
    && linkIdentityMatches(link)
    && audit.updatedAt !== null
    && link.updatedAt === audit.updatedAt
    && linkHash === audit.linkCanonicalSha256
    && sameStrings(link.sourceIds, BIRTH2M_GROSS_MOTOR_TARGET.desiredSourceIds));
  const mediaExact = await rowsMatch(mediaRows, BIRTH2M_GROSS_MOTOR_MEDIA_PREIMAGES);
  const reviewsExact = await rowsMatch(reviewRows, BIRTH2M_GROSS_MOTOR_REVIEW_PREIMAGES);
  const desiredRevisionApprovals = reviewRows.filter((row) => (
    row.reviewRevision === BIRTH2M_GROSS_MOTOR_TARGET.contentDesiredReviewRevision
  ...
  const outstandingRequiredReviews = BIRTH2M_GROSS_MOTOR_REQUIRED_REVISION_4_REVIEWS
  ...
  if (contentRows.length !== 1) blockers.push('birth-to-2-month gross-motor content row count is not one');
  if (linkRows.length !== 1) blockers.push('birth-to-2-month gross-motor link row count is not one');
  if (!mediaExact) blockers.push('birth-to-2-month gross-motor media preimage drifted');
  if (!reviewsExact) blockers.push('birth-to-2-month gross-motor review history drifted');
  if (desiredRevisionApprovals !== 0) {
    blockers.push('birth-to-2-month gross-motor revision 4 already has approvals');
  }
  if (aiReleaseRows.length !== 0) blockers.push('birth-to-2-month gross-motor content gained an AI release');
  if (aiContentAuditRows.length !== 0) {
    blockers.push('birth-to-2-month gross-motor content gained an AI content audit');
  }
  if (!citationEligible) blockers.push('desired birth-to-2-month gross-motor citation is not publication-eligible');
  if (!sameStrings(eligibleDesiredSourceIds, BIRTH2M_GROSS_MOTOR_TARGET.desiredSourceIds)) {
    blockers.push('eligible desired birth-to-2-month gross-motor citation set drifted');
  }
  ...
  if (blockers.length === 0 && audit.rows === 1 && audit.exact
    ...
    if (audit.rows === 0 && !contentInitialMatches) {
      blockers.push('production birth-to-2-month gross-motor content preimage drifted');
    }
    if (audit.rows === 0 && !linkInitialMatches) {
      blockers.push('production birth-to-2-month gross-motor link preimage drifted');
    }
    if (audit.rows === 0 && link
      && sameStrings(link.sourceIds, BIRTH2M_GROSS_MOTOR_TARGET.desiredSourceIds)) {
      blockers.push('desired birth-to-2-month gross-motor link exists without the release audit');
    }
    if (audit.rows === 1 && audit.exact && (!contentDesiredMatches || !linkDesiredMatches)) {
      blockers.push('release audit exists but birth-to-2-month gross-motor postimage drifted');
    }
  }
...
/** Read-only exact-state preflight for the birth-to-2-month gross-motor milestone. */
export const preflight = internalQuery({
  args: { releaseId: v.literal(BIRTH2M_GROSS_MOTOR_CAS_RELEASE_ID) },
  returns: preflightResultValidator,
  handler: async (ctx) => preflightState(ctx, Date.now()),
});

/** Atomically corrects the content/citations and starts an unapproved revision 4. */
export const apply = internalMutation({
  ...
  handler: async (ctx) => {
    await assertNoPersistedReleaseGovernedContent(ctx, [BIRTH2M_GROSS_MOTOR_TARGET.slug]);
    const now = Date.now();
    ...
    if (before.phase !== 'ready') {
      throw new Error(`Birth-to-2-month gross-motor CAS preflight blocked: ${before.blockers.join('; ')}`);
    }

    const rechecked = await preflightState(ctx, now);
    if (rechecked.phase !== 'ready') {
      throw new Error('Birth-to-2-month gross-motor state changed after preflight');
    }

    await ctx.db.patch(
      BIRTH2M_GROSS_MOTOR_TARGET.contentId as Id<'libraryContent'>,
      {
    ...
    await ctx.db.patch(
      BIRTH2M_GROSS_MOTOR_TARGET.linkId as Id<'evidenceLinks'>,
      {
        sourceIds: [...BIRTH2M_GROSS_MOTOR_TARGET.desiredSourceIds],
        updatedAt: now,
      },
    ...
    const [contentAfter, linkAfter] = await Promise.all([
      ctx.db.get(BIRTH2M_GROSS_MOTOR_TARGET.contentId as Id<'libraryContent'>),
      ctx.db.get(BIRTH2M_GROSS_MOTOR_TARGET.linkId as Id<'evidenceLinks'>),
    ]);
    if (!contentAfter || !linkAfter) throw new Error('Birth-to-2-month gross-motor postimage row disappeared');
    ...
    if (after.phase !== 'applied') {
      throw new Error('Birth-to-2-month gross-motor CAS postimage validation failed; transaction rolled back');
    }
    return {
      releaseId: BIRTH2M_GROSS_MOTOR_CAS_RELEASE_ID,
      applied: true,
