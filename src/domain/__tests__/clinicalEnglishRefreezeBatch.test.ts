...
describe('English all-14 refreeze batch', () => {
  const registration = CLINICAL_REVIEW_BATCH_REGISTRY.find(
    (entry) => entry.manifest.batchId === CLINICAL_ENGLISH_REFREEZE_BATCH_ID,
  );

  it('is the exact sequence-9 refreeze successor of the stopped English batch', () => {
    ...
    expect(CLINICAL_ENGLISH_REFREEZE_BATCH_PREIMAGES.expectedDecisionSetDigest)
      .toBe(CLINICAL_ENGLISH_REFREEZE_DECISION_SET_DIGEST);
  });

  it('binds the same fourteen exact post-correction revisions to Daw La Pyae', () => {
    expect(CLINICAL_ENGLISH_REFREEZE_BATCH_REVIEWER).toMatchObject({
    ...
    expect(CLINICAL_ENGLISH_REFREEZE_BATCH_ITEMS).toHaveLength(14);
    expect(CLINICAL_ENGLISH_REFREEZE_BATCH_ITEMS.map((item) => item.reviewRevision))
      .toEqual([5, 6, 10, 5, 10, 9, 9, 9, 9, 8, 8, 8, 8, 8]);
    expect(CLINICAL_ENGLISH_REFREEZE_BATCH_ITEMS.map((item) => item.ordinal))
      .toEqual(Array.from({ length: 14 }, (_, index) => index + 1));
    for (const item of CLINICAL_ENGLISH_REFREEZE_BATCH_ITEMS) {
  ...
  it('regenerates the exact manifest and routing digests', async () => {
    expect(await sha256Canonical(CLINICAL_ENGLISH_REFREEZE_BATCH_MANIFEST))
    ...
    expect(await sha256Canonical(clinicalReviewBatchRoutingPayload(registration!)))
      .toBe(CLINICAL_ENGLISH_REFREEZE_BATCH_ROUTING_HASH);
  });

  it('preserves the committed frozen git base when the audit fixture is rewritten', () => {
    const auditSource = readFileSync(
      resolve(process.cwd(), 'scripts/auditClinicalEnglishRefreezeBatch.mts'),
      'utf8',
    );
