...
vi.mock('../../../convex/lib/seedPolicy', async (importOriginal) => ({
  ...
  // Exercise the skip path for all three, including an adversarial future
...
import {
  BOOK_ACTIVITY_ATTRIBUTION_CORRECTION_SLUGS,
  isBookActivityAttributionCorrectionSlug,
} from '../../../convex/lib/bookActivityAttributionCorrection';
...
describe('exact book-activity attribution importer guards', () => {
  it('protects only the three exact slugs, regardless of the supplied type', () => {
    expect(BOOK_ACTIVITY_ATTRIBUTION_CORRECTION_SLUGS).toEqual(fixtures.slugs);
    for (const slug of fixtures.slugs) {
      expect(isBookActivityAttributionCorrectionSlug(slug)).toBe(true);
      for (const type of ['activity', 'guide', 'story', 'unexpected']) {
        expect(seedRunSkipsItem({ type, slug })).toBe(true);
      }
      expect(isBookActivityAttributionCorrectionSlug(`${slug}_other`)).toBe(false);
    }
    expect(seedRunSkipsItem({ type: 'activity', slug: fixtures.unrelatedSlug })).toBe(false);
    expect(isBookActivityAttributionCorrectionSlug('')).toBe(false);
  });
  ...
  it('retains unrelated library import behaviour while protecting the three corrections', async () => {
    const ctx = context();
    const before = protectedSnapshot(ctx);
  ...
  it('bundled seed.run skips all three before changing body, media or historical review state', async () => {
    const ctx = context();
    const before = protectedSnapshot(ctx);
  ...
  it.each(['2026-07-28-content-remediation', '2026-07-28-myanmar-copy-clarity'])(
    'historical errata %s cannot restore stale attribution while preserving unrelated behaviour', async (releaseId) => {
      ...
      await expect(handler(applyPublishedErrata)(ctx, {
        releaseId,
      })).resolves.toEqual({ updated: 1, unchanged: 3, missing: 0, notPublished: 0, total: 4 });
      expect(protectedSnapshot(ctx)).toEqual(before);
      expect(ctx.db.patch).toHaveBeenCalledTimes(1);
      expect(ctx.db.patch.mock.calls[0][0]).toBe(`content-${fixtures.unrelatedSlug}`);
      expect(ctx.db.insert.mock.calls.every(([table]) => table === 'auditLogs')).toBe(true);
