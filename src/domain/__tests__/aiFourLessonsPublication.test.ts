...
} from "../../../convex/aiFourLessonsPublication20260910";
import {
  FOUR_LESSONS_ARTIFACT as artifact,
  FOUR_LESSONS_ARTIFACT_HASH as artifactHash,
} from "../../../convex/lib/aiFourLessonsPublication20260910Artifact";
import {
...
} from "../../../convex/lib/aiFourLessonsPublication20260910Data";
...
async function fixture() {
  ...
  const scheduler = {
    runAt: vi.fn(async (time: number, _ref: unknown, args: Row) => {
      const id = "four-lesson-expiry";
      schedules.set(id, {
        _id: id,
        name: "aiFourLessonsPublication20260910.js:expire",
        scheduledTime: time,
        state: { kind: "pending" },
        args: [args],
      });
...
describe("atomic exact four-lesson AI lifecycle", () => {
  ...
  it("stages all four dark then activates all four with one schedule, no human writes and idempotent replay", async () => {
    ...
    expect(f.tables.aiAuditRuns).toHaveLength(12);
    expect(f.tables.aiContentAudits).toHaveLength(4);
    expect(f.tables.aiEvidenceAudits).toHaveLength(8);
    ...
    vi.spyOn(Date, "now").mockReturnValue(FOUR_LESSONS_VISIBILITY_CUTOFF);
  ...
  it.each([
    ["lsn_talk_more", "nhs-learn-to-talk-2023"],
    ["lsn_talk_more", "tb-paul-language-6e-2024"],
    ["lsn_making_friends", "nice-ph40-social-emotional-2012"],
    ["lsn_creativity", "who-care-for-child-development-2012"],
    ["lsn_prepare_preschool", "tb-handbook-ecse-2016"],
    ["lsn_prepare_preschool", "nice-ph40-social-emotional-2012"],
  ] as const)(
  ...
  it("rolls back the first target when second-target write fails", async () => {
    ...
    f.ctx.db.patch.mockImplementation(async (id, fields) => {
      if (id === preimage.targets[3].contentId)
        throw new Error("Injected fourth-target failure");
  ...
  it("withdraws all four exactly, preserves a newer pointer and cannot reactivate", async () => {
    const f = await fixture();
    await f.transaction(stage, operator);
    await f.transaction(activate, {
      operator: "Synthetic",
    ...
    f.targets[3].content.aiPublicationReleaseId = "newer";
    expect(await f.transaction(withdraw)).toEqual({
      revoked: 4,
      pointersCleared: 3,
    });
    expect(f.targets[3].content.aiPublicationReleaseId).toBe("newer");
    await expect(
