...
import { POWER_OF_PLAY_PREIMAGE as preimage, POWER_OF_PLAY_RELEASE_ROOT as root, POWER_OF_PLAY_RELEASE_DAYS as releaseDays, POWER_OF_PLAY_SLUG as slug } from '../../../convex/lib/aiPowerOfPlayPublication20260910Data';
...
async function fixture(previous = false) {
  ...
  async function addLane(a: Artifact, hash: string, days: number) {
    for (const target of a.targets) {
      ...
      const revision = target.slug === 'lsn_early_math' ? 11 : target.slug === 'lsn_reading_together' ? 4 : 3;
      const content: Row = {
...
describe('exact power-of-play schema2 visibility', () => {
  it('pins the real artifact and accepts only its exact registered lesson', async () => {
    ...
    expect(preimage.desiredRevision).toBe(3);
    expect(artifact.targets[0].sources).toHaveLength(2);
    expect(artifact.targets[0].mediaCount).toBe(1);
    expect(registeredAiPublicationArtifact(releaseId, artifactHash)?.kind).toBe('power_of_play');
  ...
  it('reads exact r3 with two separately audited sources without human approvals or writes', async () => {
  ...
  it('keeps all prior eleven actually readable alongside the twelfth with complete audits', async () => {
    const f = await fixture(true);
    const before = structuredClone(f.tables);
    expect((await activeAiPublicationControl(f.ctx as never)).releases).toHaveLength(12);
  ...
  it.each(['copy', 'revision', 'type', 'status', 'human_summary', 'pointer', 'timestamp', 'policy',
    'link_order', 'link_duplicate', 'source_missing', 'source_duplicate', 'source_drift', 'source_retired',
