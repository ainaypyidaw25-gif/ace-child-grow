...
import { FOUR_LESSONS_ARTIFACT as artifact, FOUR_LESSONS_ARTIFACT_HASH as artifactHash } from '../../../convex/lib/aiFourLessonsPublication20260910Artifact';
import { FOUR_LESSONS_AGGREGATE_ACTIVE_CAP as aggregateActiveCap, FOUR_LESSONS_PREIMAGE as preimages, FOUR_LESSONS_RELEASE_ROOT as root, FOUR_LESSONS_RELEASE_DAYS as releaseDays } from '../../../convex/lib/aiFourLessonsPublication20260910Data';
...
const firstSlug = 'lsn_talk_more';
const firstReleaseId = `${root}:lesson:${firstSlug}`;
const exactTargets = [
...
describe('exact additive four-lesson schema2 visibility', () => {
  it('pins the compiled artifact to the exact slugs, revisions, source order and media', async () => {
    expect(await realHash(artifact)).toBe(artifactHash);
    expect(artifact.targets).toHaveLength(4);
    expect(preimages.targets).toHaveLength(4);
    ...
    expect(artifact.targets.map(target => target.mediaCount)).toEqual([1, 1, 1, 1]);
    for (const target of exactTargets) {
      const id = `${root}:lesson:${target.slug}`;
      expect(registeredAiPublicationArtifact(id, artifactHash)?.kind).toBe('four_lessons');
      expect(registeredAiPublicationArtifact(id.replace(':lesson:', ':story:'), artifactHash)).toBeNull();
    }
    expect(registeredAiPublicationArtifact(`${root}:lesson:lsn_big_feelings`, artifactHash)).toBeNull();
    expect(registeredAiPublicationArtifact(firstReleaseId, 'f'.repeat(64))).toBeNull();
    expect(isAiPublicationTarget('lesson', firstSlug)).toBe(false);
    expect(AI_PUBLICATION_MAX_ACTIVE_RELEASES).toBe(3);
    expect(aggregateActiveCap).toBe(18);
  });

  it.each(['duplicate', 'unknown', 'extra', 'order', 'revision', 'source', 'media', 'missing_report'] as const)(
    'rejects compiled four-lesson scope drift: %s', drift => {
      ...
      try {
        ...
        if (drift === 'duplicate') mutableArtifact.targets[3] = structuredClone(mutableArtifact.targets[0]);
        if (drift === 'unknown') mutableArtifact.targets[3].slug = 'lsn_big_feelings';
        ...
        if (drift === 'revision') mutablePreimages.targets[3].desiredRevision = 3;
        if (drift === 'source') (mutableArtifact.targets[3].sources as Row[])[0].sourceId = 'jr-weisleder-2013';
        if (drift === 'media') mutableArtifact.targets[3].mediaCount = 2;
        if (drift === 'missing_report') mutableArtifact.targets[3].independentAgentResults = [];
  ...
  it('reads all four exact targets without human approvals or writes', async () => {
    ...
    expect(control.releases).toHaveLength(4);
  ...
  it('keeps the full synthetic prior fourteen plus the additive four readable', async () => {
    const f = await fixture(true);
    const before = structuredClone(f.tables);
    const control = await activeAiPublicationControl(f.ctx as never);
    expect(control.complete).toBe(true);
    expect(control.releases).toHaveLength(18);
    ...
    'fails dark for %s active control', async drift => {
      ...
      if (drift === 'unknown') {
        f.tables.aiPublicationReleases.push({
          ...
          releaseId: 'unknown',
          contentType: 'lesson',
          contentSlug: 'lsn_unknown',
          targetKey: 'lesson\0lsn_unknown',
        });
