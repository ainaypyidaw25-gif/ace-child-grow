...
const fixtures = vi.hoisted(() => {
  const slugs = ['lsn_power_of_play'];
  const unrelatedSlug = 'lesson_unrelated_play_guard_fixture';
  return {
    slugs,
    unrelatedSlug,
    items: [...slugs, unrelatedSlug].map((slug, index) => ({
      type: index === 0 ? 'unexpected' : 'lesson',
      slug,
      titleEn: 'Stale play title',
      titleMm: 'Stale play title',
      tags: [],
      source: 'Old source',
      version: 1,
      clinicalStatus: 'clinical_review',
      data: { body: { en: 'Stale play copy', mm: 'Stale play copy' } },
...
import {
  POWER_OF_PLAY_SLUG,
  isPowerOfPlaySlug,
} from '../../../convex/lib/powerOfPlayScope';
...
function context(status = 'clinical_review', includeExisting = true) {
  const tables: Record<string, Row[]> = {
    ...
    libraryContent: includeExisting ? fixtures.items.map((item) => ({
      ...
      data: { body: { en: 'Corrected exact play', mm: 'Corrected exact play' } },
    ...
    contentReviews: fixtures.slugs.map((slug) => ({
      _id: `review-${slug}`, contentSlug: slug, reviewRevision: 2,
      contentVersion: 2, dimension: 'evidence', decision: 'approved',
    })),
  };
...
describe('exact power-of-play correction importer guards', () => {
  ...
  it.each(['draft', 'clinical_review', 'approved', 'published'])(
    ...
    async (status) => {
      ...
      await expect(handler(importSeed)(ctx, { items: fixtures.items.slice(0, 1) })).resolves.toEqual({
        created: 0, updated: 0, skippedApproved: 1, total: 1,
      });
  ...
  it('keeps unrelated public import behavior intact', async () => {
    ...
    await expect(handler(importSeed)(ctx, { items: fixtures.items })).resolves.toEqual({
      created: 0, updated: 1, skippedApproved: 1, total: 2,
    });
    expect(protectedSnapshot(ctx)).toEqual(before);
  ...
  it.each([true, false])('seed.run preserves protected rows or their absence (existing=%s)', async (existing) => {
    ...
    await expect(handler(run)(ctx, {})).resolves.toEqual({
      created: existing ? 0 : 1, updated: existing ? 1 : 0, skippedApproved: 1, total: 2,
    });
    expect(protectedSnapshot(ctx)).toEqual(before);
  ...
  it.each(['2026-07-28-content-remediation', '2026-07-28-myanmar-copy-clarity'])(
    'historical errata %s cannot restore stale play copy', async (releaseId) => {
      const ctx = context('published');
      const before = protectedSnapshot(ctx);
      await expect(handler(applyPublishedErrata)(ctx, { releaseId })).resolves.toEqual({
        updated: 1, unchanged: 1, missing: 0, notPublished: 0, total: 2,
      });
      expect(protectedSnapshot(ctx)).toEqual(before);
      expect(ctx.db.patch).toHaveBeenCalledTimes(1);
