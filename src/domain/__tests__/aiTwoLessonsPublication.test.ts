...
} from "../../../convex/aiTwoLessonsPublication20260910";
import {
  TWO_LESSONS_ARTIFACT as artifact,
  TWO_LESSONS_ARTIFACT_HASH as artifactHash,
} from "../../../convex/lib/aiTwoLessonsPublication20260910Artifact";
import {
...
} from "../../../convex/lib/aiTwoLessonsPublication20260910Data";
...
async function fixture() {
  ...
  vi.spyOn(visibility, "contentIsAiParentReadable").mockImplementation(
    async (_ctx, row) =>
      ...
      row.aiPublicationReleaseId === root + ":lesson:" + row.slug,
  );
...
describe("atomic exact two-lesson AI lifecycle", () => {
  ...
  it("stages both dark then activates both with one schedule, no human writes and idempotent replay", async () => {
    const f = await fixture();
    expect(await f.transaction(preflight, { checkedAt: now })).toMatchObject({
      phase: "ready",
      activeReleaseCount: 12,
    });
    ...
    expect(f.tables.aiContentAudits).toHaveLength(2);
    ...
    vi.spyOn(Date, "now").mockReturnValue(TWO_LESSONS_VISIBILITY_CUTOFF);
  ...
  it.each([
    ...
    async (kind) => {
      ...
      if (kind === "media")
        t.media.url = "https://example.invalid/unreviewed.png";
      if (kind === "review")
  ...
  it.each(["nice-ph40-social-emotional-2012", "who-nurturing-care-2018"])(
    "rejects unvalidated removed-source audit %s when pinned",
    async (sourceId) => {
      const f = await fixture();
      const target = preimage.targets.find((t) =>
        t.sourceRows.some((s) => s.sourceId === sourceId),
      );
      expect(target).toBeDefined();
      f.tables.aiEvidenceAudits.push({
        _id: "unvalidated",
        sourceId,
        runId: root + ":evidence:" + target!.slug + ":" + sourceId,
      });
      await expect(f.transaction(stage, operator)).rejects.toThrow(
        /preservation/,
      );
  ...
  it("rolls back the first target when second-target write fails", async () => {
    ...
    f.ctx.db.patch.mockImplementation(async (id, fields) => {
      if (id === preimage.targets[1].contentId)
  ...
  it("withdraws both exactly, preserves a newer pointer and cannot reactivate", async () => {
    const f = await fixture();
    await f.transaction(stage, operator);
    await f.transaction(activate, {
      operator: "Synthetic",
    ...
    f.targets[1].content.aiPublicationReleaseId = "newer";
    expect(await f.transaction(withdraw)).toEqual({
      revoked: 2,
      pointersCleared: 1,
    });
    expect(f.targets[1].content.aiPublicationReleaseId).toBe("newer");
    await expect(
