...
import { apply, preflightAt } from '../../../convex/englishRefreezeCorrection';
import {
...
function exactContext() {
  const fixture = ENGLISH_REFREEZE_CORRECTION_PREIMAGES;
  ...
  const insert = vi.fn(async (table: string, value: Row) => {
    const id = `${table}:english-refreeze:${++inserted}`;
...
const beforeExpiry = ENGLISH_REFREEZE_CORRECTION_EXPIRES_AT - 60_000;

afterEach(() => vi.restoreAllMocks());

describe('English 14-item refreeze correction handlers', () => {
    ...
    ['stopped decision', (state: ReturnType<typeof exactContext>) => {
      const decision = state.tables.contentReviews.find(
        (row) => row.clinicalReviewBatchId
          === ENGLISH_REFREEZE_CORRECTION_PREIMAGES.registry.batches
  ...
  it('atomically advances all 14 revisions, changes only confirmed copy, and replays after expiry', async () => {
    ...
    expect(state.tables.auditLogs[0]).toMatchObject({
      action: ENGLISH_REFREEZE_CORRECTION_ACTION,
      summary: ENGLISH_REFREEZE_CORRECTION_RELEASE_ID,
      result: 'ok',
    });
    ...
    const nutrition = state.tables.libraryContent.find(
      (row) => row.slug === 'gd_5_6m_nutrition',
    )!;
    const nutritionData = nutrition.data as {
      title: { mm: string };
      why: { mm: string };
    };
    expect(nutrition.titleMm).toBe(ENGLISH_REFREEZE_CONFIRMED_COPY.titleMm);
    expect(nutrition.summaryMm).toBe(ENGLISH_REFREEZE_CONFIRMED_COPY.whyMm);
    expect(nutritionData.title.mm).toBe(ENGLISH_REFREEZE_CONFIRMED_COPY.titleMm);
    expect(nutritionData.why.mm).toBe(ENGLISH_REFREEZE_CONFIRMED_COPY.whyMm);

    state.patch.mockClear();
    state.insert.mockClear();
    clock.mockReturnValue(ENGLISH_REFREEZE_CORRECTION_EXPIRES_AT + 60_000);
