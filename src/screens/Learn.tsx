    ...
    <div className="mx-auto max-w-5xl space-y-7">
      ...
      {!online && (
        <section className="flex items-center justify-between gap-3 rounded-2xl bg-pastel-yellow/65 px-4 py-3 text-sm text-ink" role="status">
          ...
          {!nativeStoreBuild && <Link to="/offline" className="shrink-0 font-bold text-sky-deep underline">{L('သိမ်းထားသည်များ', 'Downloads')}</Link>}
        </section>
      )}
                ...
                <li key={item._id}>
                  <Link to={`/content/${item.slug}`} className="group flex items-start gap-4 px-4 py-5 sm:px-5">
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <AiPublicationStatus publicationLane={item.publicationLane} locale={locale} compact />
      ...
      <section className={`grid gap-3 ${nativeStoreBuild ? '' : 'sm:grid-cols-2'}`}>
        ...
        {!nativeStoreBuild && <Link to="/offline" className="rounded-[26px] bg-lavender/35 p-5 text-ink">
