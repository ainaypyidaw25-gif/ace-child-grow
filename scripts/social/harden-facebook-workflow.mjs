...
const quiet = new Set(['NO_DUE_APPROVED_ITEM']);
return [{ json: { ...(item || {}), dueCount: due.length, destinationPageId: manifest.destination?.pageId, graphApiVersion: manifest.destination?.graphApiVersion, eligible: blockers.length === 0, blockers, alertOwner: !killSwitchOn && blockers.some((b) => !quiet.has(b)) } }];`
...
return [{json:{action:'CLAIM',...base,status:'reserved',reservationToken:executionId,reservationExpiresAt:new Date(now+600000).toISOString(),platformPostId:'',platformPermalink:'',publishedAt:'',blockers:[],alertOwner:false}}];`, 2480, -160),
  ...
  ifNode('ready-to-claim', 'Ready to Claim Publish Slot?', '={{ $json.action }}', 'CLAIM', 2920, -120),
  {
    ...
    ], { status: 'reserved', reservationToken: '={{ $json.reservationToken }}', reservationExpiresAt: '={{ $json.reservationExpiresAt }}', lastExecutionId: '={{ $json.lastExecutionId }}' }, 3140, -120),
  ...
  dataTable('upsert-published-ledger', 'Upsert Published Ledger', 'upsert', [{ keyName: 'ledgerKey', condition: 'eq', keyValue: '={{ $json.ledgerKey }}' }], { ledgerKey: '={{ $json.ledgerKey }}', postId: '={{ $json.postId }}', status: '={{ $json.status }}', reservationToken: '={{ $json.reservationToken }}', reservationExpiresAt: '={{ $json.reservationExpiresAt }}', approvedContentHash: '={{ $json.approvedContentHash }}', mediaSha256: '={{ $json.mediaSha256 }}', platformPostId: '={{ $json.platformPostId }}', platformPermalink: '={{ $json.platformPermalink }}', publishedAt: '={{ $json.publishedAt }}', scheduledAt: '={{ $json.scheduledAt }}', lastExecutionId: '={{ $json.lastExecutionId }}', alertCode: '={{ $json.alertCode }}', alertMessage: '={{ $json.alertMessage }}' }, 4260, -80),
  ...
  dataTable('upsert-owner-alert', 'Upsert Durable Owner Alert', 'upsert', [{ keyName: 'ledgerKey', condition: 'eq', keyValue: '={{ $json.ledgerKey }}' }], { ledgerKey: '={{ $json.ledgerKey }}', postId: '={{ $json.postId }}', status: '={{ $json.status }}', reservationToken: '', reservationExpiresAt: '', approvedContentHash: '', mediaSha256: '', platformPostId: '', platformPermalink: '', publishedAt: '', scheduledAt: '', lastExecutionId: '={{ $json.lastExecutionId }}', alertCode: '={{ $json.alertCode }}', alertMessage: '={{ $json.alertMessage }}' }, 1600, 260),
