...
} from '../convex/lib/clinicalNativeMyanmarSuccessorBatchData.ts';
import {
  CLINICAL_ENGLISH_REFREEZE_BATCH_ITEMS,
  CLINICAL_ENGLISH_REFREEZE_BATCH_REVIEWER,
} from '../convex/lib/clinicalEnglishRefreezeBatchData.ts';

const deployment = 'graceful-possum-566';
const deploymentReference = 'hotel-ace-groups-of-company:ace-child-grow:prod';
const batchId = 'clinical-english-successor-14-2026-08-31-v1';
const sequence = 13;
const dimension = 'english' as const;
const fixturePath = resolve(
  process.cwd(),
  'convex/lib/clinicalEnglishSuccessorBatchPreimages.json',
);
const writeFixture = process.argv.includes('--write');
const targets = CLINICAL_NATIVE_MYANMAR_SUCCESSOR_BATCH_ITEMS.map((item) => ({
  kind: item.kind,
  slug: item.slug,
...
const query = `
  ...
  return {
    ...
    reviewerProfiles: await ctx.db.query("parentProfiles")
      .withIndex("by_user", (q) => q.eq("userId", ${JSON.stringify(CLINICAL_ENGLISH_REFREEZE_BATCH_REVIEWER.userId)})).take(2),
    batches,
    assignments,
    receipts,
    batchDecisions,
...
const items = production.targets.map((snapshot, index) => {
  const base = CLINICAL_NATIVE_MYANMAR_SUCCESSOR_BATCH_ITEMS[index];
  const advisory = CLINICAL_ENGLISH_REFREEZE_BATCH_ITEMS.find(
    (candidate) => candidate.slug === base.slug,
  )?.reviewerAdvisory;
  ...
  const currentDimension = english.filter((row) =>
    row.contentVersion === content?.reviewRevision || row.reviewRevision === content?.reviewRevision);
...
const predecessorBatchRows = production.batches.filter(
  (row) => row.batchId === CLINICAL_NATIVE_MYANMAR_SUCCESSOR_BATCH_ID,
);
...
const predecessorDecisions = production.targets.flatMap((snapshot) => snapshot.reviews.filter(
  (row) => row.clinicalReviewBatchId === CLINICAL_NATIVE_MYANMAR_SUCCESSOR_BATCH_ID,
));
...
const expectedPreviousReceiptDigest = committedFixture?.expectedPreviousReceiptDigest
  ?? predecessorReceipt?.receiptDigest;
const predecessorExact = predecessorBatchRows.length === 1
  && predecessorBatchRows[0].status === 'completed'
  && predecessorBatchRows[0].freezeDigest === CLINICAL_NATIVE_MYANMAR_SUCCESSOR_BATCH_HASH
  ...
  && predecessorReceipt?.freezeDigest === CLINICAL_NATIVE_MYANMAR_SUCCESSOR_BATCH_HASH
...
const routing = {
  ...
  activation: {
    kind: 'after_handoff',
    previousBatchId: CLINICAL_NATIVE_MYANMAR_SUCCESSOR_BATCH_ID,
...
const snapshotExact = items.every(({ exact }) => exact)
  ...
  && sha256(stableIdentity)
    === CLINICAL_ENGLISH_REFREEZE_BATCH_REVIEWER.identityCanonicalSha256
  && predecessorExact
  && activeBatches.length === 0
  && newBatchAbsent
  && globalRegistryUnique
  && production.batches.length === 11
  && production.assignments.length === 114
  && production.receipts.length === 7
