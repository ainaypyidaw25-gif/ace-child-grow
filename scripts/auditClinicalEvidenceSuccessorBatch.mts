...
} from '../convex/lib/clinicalEnglishSuccessorBatchData.ts';
import {
...
const batchId = 'clinical-evidence-successor-14-2026-09-01-v1';
const sequence = 14;
const dimension = 'evidence' as const;
const fixturePath = resolve(
  process.cwd(),
  'convex/lib/clinicalEvidenceSuccessorBatchPreimages.json',
);
const writeFixture = process.argv.includes('--write');
const targets = CLINICAL_ENGLISH_SUCCESSOR_BATCH_ITEMS.map((item) => ({
  kind: item.kind,
  slug: item.slug,
  sourceIds: item.sourceIds,
}));

const EVIDENCE_REVIEW_ADVISORY = {
  mm: 'ကိုးကားထားသော ရင်းမြစ်တိုင်းသည် အကြောင်းအရာ၏ အဆိုများကို တိုက်ရိုက်ပံ့ပိုးပြီး လက်ရှိအသုံးပြုရန် သင့်လျော်ကာ အာဏာပိုင် သို့မဟုတ် မူရင်းရင်းမြစ်ဖြစ်ကြောင်း စစ်ဆေးပါ။ အဆိုတစ်ခုသည် ရင်းမြစ်က မပံ့ပိုးပါက သို့မဟုတ် ရင်းမြစ်၏ အတိုင်းအတာကို ကျော်လွန်ပါက approve မလုပ်ဘဲ changes requested ရွေးပါ။',
  en: 'Confirm that every cited source directly supports the content claims, is current enough for use, and is an authoritative or primary source. If a claim is unsupported or exceeds the source scope, request changes instead of approving.',
} as const;
...
const items = production.targets.map((snapshot, index) => {
  const base = CLINICAL_ENGLISH_SUCCESSOR_BATCH_ITEMS[index];
  const advisory = EVIDENCE_REVIEW_ADVISORY;
  ...
  const currentDimension = reviews.filter((row) => row.dimension === 'evidence').filter((row) =>
    row.contentVersion === content?.reviewRevision || row.reviewRevision === content?.reviewRevision);
...
const predecessorBatchRows = production.batches.filter(
  (row) => row.batchId === CLINICAL_ENGLISH_SUCCESSOR_BATCH_ID,
);
...
const predecessorExact = predecessorBatchRows.length === 1
  ...
  && predecessorReceipt?.freezeDigest === CLINICAL_ENGLISH_SUCCESSOR_BATCH_HASH
...
const routing = {
  ...
  activation: {
    kind: 'after_handoff',
    previousBatchId: CLINICAL_ENGLISH_SUCCESSOR_BATCH_ID,
...
const snapshotExact = items.every(({ exact }) => exact)
  ...
  && production.batches.length === 12
  && production.assignments.length === 128
  && production.receipts.length === 8
