...
const built = await build({
  stdin: {
    contents: `export * from './convex/aiFourLessonsPublication20260910'; export * from './convex/lib/aiFourLessonsPublication20260910Data'; export * from './convex/lib/aiFourLessonsPublication20260910Artifact'; export * from './convex/lib/aiPublicationVisibility'; export * from './convex/lib/aiAuditHash';`,
...
let now =
  Math.max(packet.capturedAt, m.FOUR_LESSONS_ARTIFACT.auditCompletedAt) + 1000;
...
const identity = {
  releaseRoot: m.FOUR_LESSONS_RELEASE_ROOT,
  artifactHash: m.FOUR_LESSONS_ARTIFACT_HASH,
  snapshotSha256: m.FOUR_LESSONS_PREIMAGE.snapshotSha256,
};
...
try {
  ...
  for (const [slug, sourceId] of [
    ...
    tables.aiEvidenceAudits.push({
      ...
      _id: id,
      sourceId,
      runId: m.fourLessonsSourceRunId(slug, sourceId),
    });
  ...
  state = await run(m.stage, {
    ...
    gitCommit: "c75d2af1d6023e4013a2d379256ef9439d744a2b",
  });
  ...
  now = m.FOUR_LESSONS_VISIBILITY_CUTOFF;
  result.expired = await run(m.expire);
  assert.deepEqual(result.expired, { revoked: 4, pointersCleared: 4 });
  for (const t of packet.targets)
    assert.equal(
  ...
  result.passed = true;
  writeFileSync(
    "artifacts/four-lessons-preview-20260910/private-lifecycle-verification.json",
