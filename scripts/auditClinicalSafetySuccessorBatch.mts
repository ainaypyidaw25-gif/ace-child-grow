...
} from '../convex/lib/clinicalEvidenceSuccessorBatchData.ts';
import {
...
const batchId = 'clinical-safety-successor-14-2026-09-01-v1';
const sequence = 15;
const dimension = 'safety' as const;
const fixturePath = resolve(
  process.cwd(),
  'convex/lib/clinicalSafetySuccessorBatchPreimages.json',
);
const writeFixture = process.argv.includes('--write');
const targets = CLINICAL_EVIDENCE_SUCCESSOR_BATCH_ITEMS.map((item) => ({
  kind: item.kind,
  slug: item.slug,
  sourceIds: item.sourceIds,
}));

const SAFETY_REVIEW_ADVISORY = {
  mm: 'ကလေး၏ အသက်နှင့် ဖွံ့ဖြိုးမှုအဆင့်အတွက် ဘေးကင်းရေးအကြံပြုချက်တိုင်း မှန်ကန်ကြောင်း၊ ရေ၊ လမ်း၊ မီးလောင်ဒဏ်နှင့် အိမ်တွင်းအန္တရာယ်များကို အနီးကပ်ကြီးကြပ်မှုဖြင့် လုံလောက်စွာ ကာကွယ်ထားကြောင်း စစ်ဆေးပါ။ အန္တရာယ်တစ်ခုကို လျှော့တွက်ထားပါက approve မလုပ်ဘဲ changes requested ရွေးပါ။',
  en: 'Confirm that every safety recommendation is appropriate for the child’s age and developmental stage, and that close supervision adequately addresses water, traffic, burn, and home hazards. If any risk is understated, request changes instead of approving.',
} as const;
...
const items = production.targets.map((snapshot, index) => {
  const base = CLINICAL_EVIDENCE_SUCCESSOR_BATCH_ITEMS[index];
  const advisory = SAFETY_REVIEW_ADVISORY;
  ...
  const currentDimension = reviews.filter((row) => row.dimension === 'safety').filter((row) =>
    row.contentVersion === content?.reviewRevision || row.reviewRevision === content?.reviewRevision);
...
const predecessorBatchRows = production.batches.filter(
  (row) => row.batchId === CLINICAL_EVIDENCE_SUCCESSOR_BATCH_ID,
);
...
const predecessorDecisions = production.targets.flatMap((snapshot) => snapshot.reviews.filter(
  (row) => row.clinicalReviewBatchId === CLINICAL_EVIDENCE_SUCCESSOR_BATCH_ID,
));
...
const expectedPreviousDecisionDigest = committedFixture?.expectedPreviousDecisionDigest
  ?? '28917526d547b99fe71deeb098b3c492986cf4f95c32e8b400c7a3b32a91a5cf';
const expectedPreviousReceiptDigest = committedFixture?.expectedPreviousReceiptDigest
  ?? 'cd98c905a18850b4c9fafb318e37781006e21dfbd51870d8d555f5671f2344a5';
const predecessorExact = predecessorBatchRows.length === 1
  && predecessorBatchRows[0].status === 'completed'
  && predecessorBatchRows[0].freezeDigest === CLINICAL_EVIDENCE_SUCCESSOR_BATCH_HASH
  ...
  && predecessorReceipt?.freezeDigest === CLINICAL_EVIDENCE_SUCCESSOR_BATCH_HASH
...
const routing = {
  ...
  activation: {
    kind: 'after_handoff',
    previousBatchId: CLINICAL_EVIDENCE_SUCCESSOR_BATCH_ID,
...
const snapshotExact = items.every(({ exact }) => exact)
  ...
  && production.batches.length === 13
  && production.assignments.length === 142
  && production.receipts.length === 9
