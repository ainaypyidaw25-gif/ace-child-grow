...
} from '../convex/lib/clinicalNativeMyanmarRefreezeBatchData.ts';

const deployment = 'graceful-possum-566';
const deploymentReference = 'hotel-ace-groups-of-company:ace-child-grow:prod';
const batchId = 'clinical-native-myanmar-successor-14-2026-08-31-v1';
const sequence = 12;
const dimension = 'native_myanmar' as const;
const fixturePath = resolve(
  process.cwd(),
  'convex/lib/clinicalNativeMyanmarSuccessorBatchPreimages.json',
);
const writeFixture = process.argv.includes('--write');
const targets = CLINICAL_CHILD_DEVELOPMENT_REFREEZE_BATCH_ITEMS.map((item) => ({
  kind: item.kind,
  slug: item.slug,
...
const items = production.targets.map((snapshot, index) => {
  const base = CLINICAL_CHILD_DEVELOPMENT_REFREEZE_BATCH_ITEMS[index];
  const advisory = CLINICAL_NATIVE_MYANMAR_REFREEZE_BATCH_ITEMS.find(
  ...
  const currentDimension = nativeMyanmar.filter((row) =>
    row.contentVersion === content?.reviewRevision || row.reviewRevision === content?.reviewRevision);
...
const predecessorBatchRows = production.batches.filter(
  (row) => row.batchId === CLINICAL_CHILD_DEVELOPMENT_REFREEZE_BATCH_ID,
);
...
const predecessorDecisions = production.targets.flatMap((snapshot) => snapshot.reviews.filter(
  (row) => row.clinicalReviewBatchId === CLINICAL_CHILD_DEVELOPMENT_REFREEZE_BATCH_ID,
));
...
const predecessorExact = predecessorBatchRows.length === 1
  ...
  && predecessorBatchRows[0].freezeDigest === CLINICAL_CHILD_DEVELOPMENT_REFREEZE_BATCH_HASH
  ...
  && predecessorReceipt?.freezeDigest === CLINICAL_CHILD_DEVELOPMENT_REFREEZE_BATCH_HASH
  ...
  && predecessorDecisions.every((row) =>
    row.dimension === 'child_development'
...
const routing = {
  ...
  activation: {
    kind: 'after_handoff',
    previousBatchId: CLINICAL_CHILD_DEVELOPMENT_REFREEZE_BATCH_ID,
...
const snapshotExact = items.every(({ exact }) => exact)
  ...
  && sha256(stableIdentity)
    === CLINICAL_NATIVE_MYANMAR_REFREEZE_BATCH_REVIEWER.identityCanonicalSha256
  && predecessorExact
  && activeBatches.length === 0
  && newBatchAbsent
  && globalRegistryUnique
  && production.batches.length === 10
  && production.assignments.length === 100
  && production.receipts.length === 6
