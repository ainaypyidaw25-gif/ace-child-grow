...
const slugs=['lsn_what_is_development','lsn_big_feelings'],root='2026-09-10-two-lessons-ai-preview-v1';
const selected=[['cdc-monitoring-screening-2026'],['aap-toxic-stress-2021']];
const get=async(table,index,field,key,limit=101)=>ctx.db.query(table).withIndex(index,q=>q.eq(field,key)).take(limit);
const activeReleases=await get('aiPublicationReleases','by_status','status','active',15);
if(activeReleases.length!==12)throw new Error('Expected exactly twelve existing active releases');
const preservation=[];
const keys=new Set();
...
for(const prefix of ['library.ai_early_math','library.ai_two_stories','library.ai_seven_stories','library.ai_reading_together','library.ai_power_of_play'])for(const phase of ['staged','enabled','withdrawn','expired']){
...
const sources=[];for(const id of [...new Set([...links[0].sourceIds,...selected[i]])]){const rows=await get('evidenceSources','by_source_id','sourceId',id,2);if(rows.length!==1)throw new Error('Exact source missing');sources.push(rows[0]);await pin('aiEvidenceAudits','by_source_and_updated_at','sourceId',id);}
...
for(const phase of ['staged','enabled','withdrawn','expired'])collisions.push({table:'auditLogs',index:'by_action',field:'action',key:'library.ai_two_lessons.'+phase,rows:await get('auditLogs','by_action','action','library.ai_two_lessons.'+phase,2)});
...
const packet = JSON.parse(
  execFileSync(
    "npx",
    ["--no-install", "convex", "run", "--prod", "--inline-query", query],
    {
      encoding: "utf8",
...
const directory = "artifacts/two-lessons-preview-20260910";
  ...
  JSON.stringify(
    {
      ...
      preservation: packet.preservation.length,
      schedules: packet.oldSchedules.length,
      targets: packet.targets.map((t) => ({
        slug: t.content.slug,
        revision: t.content.reviewRevision,
        sources: t.sources.length,
        reviews: t.reviews.length,
        media: t.media.length,
