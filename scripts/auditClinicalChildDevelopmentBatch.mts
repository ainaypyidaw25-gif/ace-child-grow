...
import {
  ...
  CLINICAL_ENGLISH_REFREEZE_BATCH_EXPIRES_AT,
  CLINICAL_ENGLISH_REFREEZE_BATCH_FROZEN_AT,
  CLINICAL_ENGLISH_REFREEZE_BATCH_MANIFEST,
  CLINICAL_ENGLISH_REFREEZE_BATCH_ITEMS,
  CLINICAL_ENGLISH_REFREEZE_BATCH_REVIEWER,
...
const batchId = 'clinical-child-development-governed-14-2026-08-29-v1';
const fixturePath = resolve(process.cwd(), 'convex/lib/clinicalChildDevelopmentBatchPreimages.json');
const writeFixture = process.argv.includes('--write');
const targets = CLINICAL_ENGLISH_REFREEZE_BATCH_ITEMS.map((item) => ({
  kind: item.kind,
  slug: item.slug,
...
const query = `
  ...
  return {
    ...
    reviewerProfiles: await ctx.db.query("parentProfiles")
      .withIndex("by_user", (q) => q.eq("userId", ${JSON.stringify(CLINICAL_REVIEW_BATCH_REVIEWER.userId)})).take(2),
...
const advisory = {
  mm: 'သတ်မှတ်ထားသော အသက်အရွယ်အတွက် ကျွမ်းကျင်မှု၊ အချိန်ကာလ၊ အစီအစဉ်၊ မျှော်မှန်းချက်နှင့် မိဘလုပ်ဆောင်ချက်များသည် ကလေးဖွံ့ဖြိုးမှုအရ သင့်လျော်ကြောင်း စစ်ဆေးပါ။ ပုံမှန်ကွာခြားမှုကို မထည့်သွင်းသော ကျွမ်းကျင်မှုအာမခံချက်၊ ရောဂါသတ်မှတ်ချက် သို့မဟုတ် တင်းကျပ်သော မျှော်မှန်းချက်များ မဖြစ်စေရပါ။ ဖွံ့ဖြိုးမှုဆိုင်ရာ အဓိပ္ပါယ်ပြောင်းလဲစေမည့် ပြင်ဆင်ချက်လိုပါက approve မလုပ်ဘဲ changes requested ရွေးပါ။',
  en: 'Confirm that skills, timing, sequence, expectations, and parent actions are developmentally appropriate for the stated age. Reject milestone guarantees, diagnostic wording, or rigid expectations that do not account for normal variation. If a correction would change developmental meaning, request changes instead of approving.',
};

const todayIso = new Date(production.checkedAt).toISOString().slice(0, 10);
const items = production.targets.map((snapshot, index) => {
  const prior = CLINICAL_ENGLISH_REFREEZE_BATCH_ITEMS[index];
  ...
  const currentChildDevelopment = childDevelopment.filter((row) =>
    row.contentVersion === content?.reviewRevision || row.reviewRevision === content?.reviewRevision);
  ...
  const exact = Boolean(
    ...
    && currentChildDevelopment.length === 0
...
const predecessorBatchRows = production.batches.filter(
  (row) => row.batchId === CLINICAL_ENGLISH_REFREEZE_BATCH_ID,
);
const predecessorReceipts = production.receipts.filter(
  (row) => row.batchId === CLINICAL_ENGLISH_REFREEZE_BATCH_ID,
);
const predecessorDecisions = CLINICAL_ENGLISH_REFREEZE_BATCH_ITEMS.map((item, index) => {
  const assignmentId = sha256({
    batchId: CLINICAL_ENGLISH_REFREEZE_BATCH_ID,
    batchHash: CLINICAL_ENGLISH_REFREEZE_BATCH_HASH,
    ordinal: item.ordinal,
    kind: item.kind,
    slug: item.slug,
    reviewRevision: item.reviewRevision,
    dimension: 'english',
    reviewerUserId: CLINICAL_ENGLISH_REFREEZE_BATCH_REVIEWER.userId,
  });
  const rows = production.targets[index].reviews.filter((row) => row.decisionKey === assignmentId);
  const row = rows[0];
  const exact = rows.length === 1
    && row.clinicalReviewBatchId === CLINICAL_ENGLISH_REFREEZE_BATCH_ID
    && row.contentSlug === item.slug
    && row.contentVersion === item.reviewRevision
    && row.reviewRevision === item.reviewRevision
    && row.dimension === 'english'
    && row.decision === 'approved'
    && String(row.reviewerId) === CLINICAL_ENGLISH_REFREEZE_BATCH_REVIEWER.userId
    && row.reviewerDisplayName === CLINICAL_ENGLISH_REFREEZE_BATCH_REVIEWER.displayName
    && row.reviewerQualification === undefined
    && row.reviewerRole === CLINICAL_ENGLISH_REFREEZE_BATCH_REVIEWER.role
...
const predecessorDigest = completedAt === null ? null : sha256({
  ...
  batchId: CLINICAL_ENGLISH_REFREEZE_BATCH_ID,
  freezeDigest: CLINICAL_ENGLISH_REFREEZE_BATCH_HASH,
...
const predecessorFreezeReceiptDigest = sha256({
  ...
  batchId: CLINICAL_ENGLISH_REFREEZE_BATCH_ID,
  freezeDigest: CLINICAL_ENGLISH_REFREEZE_BATCH_HASH,
  frozenAt: CLINICAL_ENGLISH_REFREEZE_BATCH_FROZEN_AT,
  expiresAt: CLINICAL_ENGLISH_REFREEZE_BATCH_EXPIRES_AT,
...
const predecessorExact = predecessorBatchRows.length === 1
  ...
  && predecessorBatchRows[0].freezeDigest === CLINICAL_ENGLISH_REFREEZE_BATCH_HASH
  && predecessorBatchRows[0].itemCount === 14
  && predecessorBatchRows[0].completedAt === completedAt
  && predecessorReceipts.length === 1
  && predecessorReceipt.freezeDigest === CLINICAL_ENGLISH_REFREEZE_BATCH_HASH
  ...
  && String(predecessorReceipt.reviewerId) === CLINICAL_ENGLISH_REFREEZE_BATCH_REVIEWER.userId;
...
const snapshotExact = items.every(({ exact }) => exact)
  ...
  && sha256(stableIdentity) === CLINICAL_REVIEW_BATCH_REVIEWER.identityCanonicalSha256
  && predecessorExact
  && activeBatches.length === 0
  && newBatchAbsent
  && globalRegistryUnique
  && production.batches.length === 8
  && production.assignments.length === 72
...
if (writeFixture) {
  if (!allExact) throw new Error('Production child-development batch snapshot is not exact');
...
console.log(JSON.stringify({
  ...
  predecessor: {
    ...
    decisions: predecessorDecisions.map((entry) => ({
      slug: entry.slug,
